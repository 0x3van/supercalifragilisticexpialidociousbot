...
function processPersonality(p) {
    ...
    const data = JSON.parse(fs.readFileSync('./prompts/' + p + '.json', 'utf-8')) // bad idea dumbass
...
class AI {
    constructor(config) { // js devs are retards
        if (!config) {
    ...
    processInput(text) {
        ...
        return new Promise(async(res, rej) => {
            ...
            const response = await this.openai.createCompletion("code-davinci-002", { // replace code with text if you do not have codex
                    ...
                    log.info("omg daddy thanks for succeeding to resoind to this shitty request")
                    res(r.data.choices[0].text)
                    this.openai.prompt = this.openai.prompt + text + '\nAI:' + r.data.choices[0].text + '\nHuman:'
                })
                .catch(() => {
                    log.error("bot throttled by shitty math function")
                    res("bot throttled by shitty math function")
                })
        })
